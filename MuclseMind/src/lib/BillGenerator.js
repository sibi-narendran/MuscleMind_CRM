...
export const generateBillingPDF = async (billing) => {
  ...
  const treatments = billing.treatment_name.split(',').map((treatment, index) => ({
    name: treatment.trim(),
  ...
  container.innerHTML = `
    <div class="w-full h-full relative bg-white" style="color: #000000;">
      ...
      <div class="w-full h-[60mm]">
        <img src="${headerSrc}" class="w-full h-full object-contain" />
        <div class="w-full h-[1px]" style="background-color: #000000;"></div>
      </div>
      ...
      <!-- Footer -->
      <div style="position: absolute; bottom: 0; width: 100%; height: 60mm;">
        <div style="width: 100%; height: 1px; background-color: #000000;"></div>
        <img src="${headerSrc}" style="width: 100%; height: 100%; object-fit: contain;" />
      </div>
    </div>
  `;
